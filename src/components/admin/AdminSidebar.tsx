...
const SECTIONS: { key: SectionKey; title: string; icon: React.ComponentType<{ className?: string }> }[] = [
        ...
        <SidebarHeader>
          <div className="flex items-center gap-2">
            ...
            <div className="grid flex-1 text-left text-sm leading-tight">
              ...
              <span className="truncate text-xs text-sidebar-foreground/60">Админ</span>
            </div>
          </div>
        </SidebarHeader>
        ...
        <SidebarFooter>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                <Avatar className="size-8 rounded-md">
                  <AvatarImage src={user?.avatar ?? ''} alt={user?.name ?? 'Админ'} />
                  <AvatarFallback className="rounded-md bg-sidebar-primary text-sidebar-primary-foreground">
                    {initials(user?.name)}
                  </AvatarFallback>
                </Avatar>
                <div className="grid flex-1 text-left text-sm leading-tight">
                  <span className="truncate font-medium">{user?.name ?? 'Администратор'}</span>
            ...
            <DropdownMenuContent side="top" align="end" className="w-56">
              <DropdownMenuLabel>{user?.email ?? 'admin@planetaup.ru'}</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuItem onSelect={(e) => e.preventDefault()}>Аккаунт</DropdownMenuItem>
